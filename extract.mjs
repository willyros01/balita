...
const JUNK = [
  ...
  /* player and duration furniture — every outlet has a version */
  /^listen to this article$/i,
  /^watch the (video|full video)$/i,
  /^estimated \d+ minutes?$/i,
  /^\d+ min(ute)? read$/i,
  /^(play|pause|replay|mute)$/i,
  /^the audio version of this article is generated by ai/i,

  /* Inquirer's subscription form leaves both of its result messages
     in the page, one of which is a failure notice nobody triggered. */
  /^your subscription could not be saved/i,
  /^your subscription has been successful/i,
  /^subscribe to our (daily )?newsletter/i,
  /^by providing an email address/i,
  ...
  /* ABS-CBN's newsletter block, which appears twice per article */
  /^abs-cbn newsletter$/i,
  /^your go-to source for the latest news, entertainment highlights/i,
  /^haven't received our newsletter\??$/i,
  /^check your inbox to verify and confirm your subscription/i,

  /* Rappler's mood widget and sign-off */
  /^how does this make you feel\??$/i,
  /^[\u2013\u2014-]\s*rappler\.com$/i,

  /* GMA's cross-link heading, which splits every article in two */
  /^other stories$/i,
  /^you may also like$/i,
  /^trending (now|stories)$/i,
...
/* A paragraph whose whole text is a web address. No outlet means one
...
function isJunk(text){
  ...
  if(BARE_URL.test(t)) return true;
...
function imageFrom(node, base){
  ...
  /* A picture captioned "Newsletter Logo" is furniture, not news. */
  if(/\b(logo|banner|advertisement|newsletter)\b/i.test(caption)) return null;
...
function toBlocks(root, base){
  ...
  /* Collapse repeats. Comparing only against the previous block was
     ...
     heading between the copies, so both survived. Compare against
     everything kept so far instead. */
  const out = [];
  const seen = new Set();
